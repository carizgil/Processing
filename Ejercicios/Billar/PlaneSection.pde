...
class PlaneSection
{
   ...
   void drawLimits(PVector p)
   {  
      stroke(255, 0, 0); // Set the color of the lines to red
      line(_pos1.x, _pos1.y, p.x, p.y); // Draw a line from _pos1 to p
      line(_pos2.x, _pos2.y, p.x, p.y); // Draw a line from _pos2 to p
      line(_pos1.x, _pos1.y, _pos2.x, _pos2.y); // Draw a line from _pos1 to _pos2
   ...
   void render()
   {
      stroke(STATIC_ELEMENTS_COLOR[0] , STATIC_ELEMENTS_COLOR[1], STATIC_ELEMENTS_COLOR[2]);
      strokeWeight(5);
